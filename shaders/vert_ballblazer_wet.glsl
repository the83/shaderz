// gen-shader
// Wetblazer: Ballblazer-style arena with a watery reflective floor
// u_x0: forward speed
...
// Layered water normal: returns XZ slope of wave surface at world pos p, time t
vec2 waterNormal(vec2 p, float t) {
    float eps = 0.06;
    // 3 layers of noise at different scales / speeds
    ...
    float h1  = vnoise(p*0.90 + vec2(-t*0.50,  t*0.38));
    float h1x = vnoise((p+vec2(eps,0))*0.90 + vec2(-t*0.50,  t*0.38));
    float h1z = vnoise((p+vec2(0,eps))*0.90 + vec2(-t*0.50,  t*0.38));

    float h2  = vnoise(p*2.10 + vec2( t*0.80, -t*0.60));
    float h2x = vnoise((p+vec2(eps,0))*2.10 + vec2( t*0.80, -t*0.60));
    float h2z = vnoise((p+vec2(0,eps))*2.10 + vec2( t*0.80, -t*0.60));

    float dx = (h0x-h0)*0.6 + (h1x-h1)*0.3 + (h2x-h2)*0.1;
    float dz = (h0z-h0)*0.6 + (h1z-h1)*0.3 + (h2z-h2)*0.1;
...
void main() {
    ...
    float asp  = u_resolution.y / u_resolution.x;
