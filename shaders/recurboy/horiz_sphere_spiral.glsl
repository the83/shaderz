// Spherical spiral — loxodrome on a rotating sphere for recurBOY
// Analytical: compute (θ,φ) per pixel, check θ ≈ a·φ via cos trick
// Front and back face checked for see-through effect
//
...
void main(void) {
    ...
    // Spiral winding density
    float a = mix(2.0, 25.0, u_x0);

    // Camera tilt (rotate view around X axis)
    ...
    // Check if pixel is inside sphere silhouette
    float r2 = pos.x * pos.x + pos.y * pos.y;
    float onSphere = 1.0 - step(1.0, r2);
    float zf = sqrt(max(1.0 - r2, 0.001));

    // Front face: apply tilt, compute spherical coords, check spiral
    ...
    // Shading: front face bright, back face dim
    ...
    // Back face drawn first (behind), front face on top
