// Brakhage — chaotic paint swirls inspired by "The Dante Quartet"
...
// Smooth bilinear noise — 4 hash calls, returns 0-1 with smooth gradients
float noise(vec2 p) {
    vec2 i = floor(p);
    vec2 f = fract(p);
    f = f * f * (3.0 - 2.0 * f);
...
// Map a smooth noise value to a blended palette color
vec3 palette(float n, float nColors, float baseHue) {
    float idx = n * nColors;
    float fi = floor(idx);
    float bl = fract(idx);
    // Smooth blend between adjacent palette slots
...
void main(void) {
    ...
    // Discrete refresh seed — each value is a new "painting"
    ...
    // Color diversity: 1-6 distinct hues per painting
    ...
    // Painterly brightness variation
