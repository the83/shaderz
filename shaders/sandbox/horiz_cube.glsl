// Rotating 3D cube — analytical ray-box intersection
...
// Face normal from hit point on unit box
vec3 boxNormal(vec3 p, vec3 size) {
    vec3 d = abs(p) - size;
    // The component closest to zero (from outside) is the face we hit
    vec3 s = sign(p);
...
// Face color based on normal axis
vec3 faceColor(vec3 n) {
    vec3 an = abs(n);
    if (an.x > 0.5) return vec3(0.9, 0.25, 0.2);  // X faces: red
    if (an.y > 0.5) return vec3(0.2, 0.8, 0.3);    // Y faces: green
    return vec3(0.2, 0.4, 0.95);                     // Z faces: blue
}

// Checkerboard pattern on face
...
// Edge highlight — darken near cube edges
float edgeFactor(vec3 p, vec3 size) {
    vec3 d = abs(p) / size;
    vec3 edge = smoothstep(0.85, 0.95, d);
    // How close are we to an edge on the two non-face axes?
...
void main(void) {
    ...
    // Per-axis rotation speeds: 0.5 = stopped
    ...
    // Rotate ray into box space (equivalent to rotating the box)
    vec3 rro = rot * ro;
    vec3 rrd = rot * rd;
